...
export async function GET(request, { params }) {
  ...
  const result = await gatewayFetchWithSession(request, `/v1/media/render-jobs/${resolvedParams.jobId}`);
  const payload = await readJson(result.gatewayResponse);
  const response = createJsonResponse(payload ?? { message: "Unable to load render job" }, {
