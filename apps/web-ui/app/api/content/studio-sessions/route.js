...
export async function POST(request) {
  ...
  const result = await gatewayFetchWithSession(request, "/v1/content/studio-sessions", {
    method: "POST",
    headers: {
  ...
  const response = createJsonResponse(payload ?? { message: "Unable to create studio session" }, {
