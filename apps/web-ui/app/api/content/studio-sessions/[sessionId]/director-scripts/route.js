...
export async function POST(request, context) {
  ...
  const result = await gatewayFetchWithSession(
    request,
    `/v1/content/studio-sessions/${resolvedParams.sessionId}/director-scripts`,
    {
      method: "POST",
      headers: {
  ...
  const response = createJsonResponse(payload ?? { message: "Unable to generate director scripts" }, {
