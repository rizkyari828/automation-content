...
export async function POST(request) {
  ...
  const result = await gatewayFetchWithSession(request, "/v1/content/template-plans:preview", {
    method: "POST",
    headers: {
  ...
  const response = createJsonResponse(payload ?? { message: "Unable to preview template plan" }, {
