...
export async function PATCH(request, context) {
  ...
  const result = await gatewayFetchWithSession(request, `/v1/content/scripts/${resolvedParams.scriptId}`, {
    method: "PATCH",
    headers: {
  ...
  const response = createJsonResponse(payload ?? { message: "Unable to update script" }, {
